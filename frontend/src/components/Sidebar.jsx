...
const navItems = [
  ...
  { icon: FaBookOpen, label: 'Books', href: '/books' },
  { icon: FaPlus, label: 'Add Book', href: '/add-book' },
];
...
const Sidebar = ({ isOpen, toggleSidebar }) => {
  ...
  return (
    <aside
      className={`
        ${theme.bg.primary} border-r ${theme.border.primary} z-30
    ...
    >
      <div className="flex flex-col flex-1 py-4 px-2 overflow-y-auto overflow-x-hidden">
        ...
        <button
          ...
          className={`${theme.text.primary} text-2xl hover:text-blue-400 mb-6 px-3 py-2 rounded-md transition self-start`}
        >
          <FaBars />
        </button>
              ...
              <div key={item.href} className="relative group">
                <Link
                  ...
                  className={`
                    ...
                    ${isActive ? 'bg-blue-600 text-white' : `${theme.text.secondary} ${theme.card.hover}`}
                ...
                {!expanded && (
                  <div className={`absolute left-full top-1/2 -translate-y-1/2 ml-2
                    ${theme.bg.secondary} ${theme.text.primary} text-xs px-2 py-1 rounded shadow-lg
